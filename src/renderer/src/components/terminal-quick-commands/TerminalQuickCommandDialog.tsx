...
export function TerminalQuickCommandDialog({
  ...
  const fallbackAgent: TuiAgent = AGENT_CATALOG[0]?.id ?? 'claude'
  ...
  const selectedAgent = isTerminalAgentQuickCommand(draft) ? draft.agent : fallbackAgent
  ...
  const saveDraft = (): void => {
    ...
    if (
      !next.label ||
      (isTerminalAgentQuickCommand(next) ? !next.prompt.trim() : !next.command.trim())
    ) {
      return
    }
    onSave(next)
